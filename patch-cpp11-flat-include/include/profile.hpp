...
public:
    ...
    bool check(double jf, double vMax, double vMin, double aMax, double aMin) {
        ...
        if (limits == Limits::ACC0_ACC1_VEL || limits == Limits::ACC0_VEL || limits == Limits::ACC1_VEL || limits == Limits::VEL) {
            if (t[3] < std::numeric_limits<double>::epsilon()) {
                return false;
            }
        }

        if (limits == Limits::ACC0 || limits == Limits::ACC0_ACC1) {
            if (t[1] < std::numeric_limits<double>::epsilon()) {
                return false;
            }
        }

        if (limits == Limits::ACC1 || limits == Limits::ACC0_ACC1) {
        ...
        for (size_t i = 0; i < 7; ++i) {
            ...
            if (limits == Limits::ACC0_ACC1_VEL || limits == Limits::ACC0_ACC1 || limits == Limits::ACC0_VEL || limits == Limits::ACC1_VEL || limits == Limits::VEL) {
                if (i == 2) {
                    a[3] = 0.0;
                }
            }
            ...
            if (set_limits) {
                if (limits == Limits::ACC1) {
                    if (i == 2) {
                        a[3] = aMin;
                    }
                }

                if (limits == Limits::ACC0_ACC1) {
                    if (i == 0) {
                        a[1] = aMax;
                    }
